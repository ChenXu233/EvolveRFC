...
flowchart TD
    subgraph 初始化
        START([开始]) --> init[初始化<br/>create_initial_state]
    end
    
    subgraph 评审阶段
        init --> parallel_review[并行评审<br/>parallel_review]
        
        subgraph parallel_review_detail["parallel_review 内部"]
            PR_start([开始]) --> PR_cleanup1[清理工具上下文]
            PR_cleanup1 --> PR_loop{"遍历评审角色"}
            
            PR_loop -- architect --> PR_start1[评审开始通知UI]
            PR_start1 --> PR_cleanup2[清理上下文]
            PR_cleanup2 --> PR_mode{"评审模式"}
            
            PR_mode -- 开启 --> PR_tools[ReAct Agent<br/>run_review_with_tools]
            PR_mode -- 关闭 --> PR_pool[单次LLM<br/>run_review_with_viewpoint_pool]
            
            PR_tools --> PR_save[保存结果]
            PR_pool --> PR_save
            
            PR_save --> PR_end[评审结束通知UI]
            PR_end --> PR_stop{"停止检查?"}
            
            PR_stop -- 是 --> PR_state[保存断点状态]
            PR_stop -- 否 --> PR_next{"下一个角色?"}
            
            PR_next -- 是 --> PR_loop
            PR_next -- 否 --> PR_events[收集评审事件]
            PR_events --> PR_vp[收集新观点]
            PR_vp --> PR_update[更新观点池]
            PR_update --> PR_end_all([结束])
        end
        
        parallel_review -.-> parallel_review_detail
    end
    
    subgraph 投票分析
        PR_end_all --> vote_analyzer[投票分析<br/>vote_analyzer]
        
        subgraph vote_analyzer_detail["vote_analyzer 内部"]
            VA_start([开始]) --> VA_collect[收集本轮投票]
            VA_collect --> VA_count[统计投票分布]
            VA_count --> VA_check{"反对票>30%?"}
            
            VA_check -- 是 --> VA_human[设置awaiting_human_input]
            VA_check -- 否 --> VA_event[添加统计事件]
        end
        
        vote_analyzer -.-> vote_analyzer_detail
    end
    ...
    subgraph 路由决策
        ...
        route -- 观点池活跃 --> VPM[观点池管理]
        route -- 无活跃观点 --> CS[书记官总结]
        route -- 人类介入 --> HO[人类监督]
    end
    
    subgraph 观点池管理
        VPM --> VPM_check{"检查活跃观点"}
        VPM_check -- 有 --> resolve[标记已解决]
        VPM_check -- 无 --> ready[准备汇总]
        resolve --> VPM_back[下一轮评审]
        ready --> VPM_back
    end
    ...
    subgraph 人类监督
        ...
        HO_decision -- 继续 --> VPM_back
        HO_decision -- 强制通过 --> FR[最终报告]
        HO_decision -- 终止 --> END([结束])
    end
    
    subgraph 总结阶段
        VPM_back --> CS[书记官总结<br/>clerk_summary]
        ready --> CS
        
        CS --> CS_update[轮次+1]
        CS_update --> CS_check{"达到最大轮次?"}
    ...
    subgraph 角色
        R1[architect<br/>架构师]
        R2[security<br/>安全专家]
        R3[cost_control<br/>成本控制]
        R4[innovator<br/>创新者]
        R5[clerk<br/>书记官]
    end
    
    parallel_review_detail --> R1 & R2 & R3 & R4
    R5 -.-> |总结| CS
    
    %% 路由器规则
    subgraph 路由器["Router 优先级"]
        Rule1[1. 紧急停止<br/>action=终止]
        Rule2[2. 人类介入<br/>awaiting_human_input]
        Rule3[3. 最大轮次<br/>current_round>=max]
        Rule4[4. 达成共识<br/>open_issues=0]
        Rule5[5. 默认汇总<br/>每轮结束]
    end
    ...
    subgraph 观点池状态
        VP_active[ACTIVE 活跃]
        VP_resolved[RESOLVED 已解决]
        VP_rejected[REJECTED 拒绝]
    end
    
    VPM_check -- 多数赞成 --> VP_resolved
    VPM_check -- 多数反对 --> VP_rejected
    
    %% 样式
    classDef start_end fill:#90EE90,stroke:#228B22,stroke-width:2px
    classDef process fill:#87CEEB,stroke:#4682B4,stroke-width:2px
    classDef decision fill:#FFD700,stroke:#DAA520,stroke-width:2px
    classDef human fill:#FFB6C1,stroke:#FF69B4,stroke-width:2px
    classDef role fill:#DDA0DD,stroke:#9370DB,stroke-width:2px
    
    class START,END,PR_end_all start_end
    class init,parallel_review,vote_analyzer,CS,VPM,HO,FR,PR_start,PR_cleanup1,PR_save,PR_events,PR_vp,PR_update,VA_start,VA_collect,VA_count,VA_event,VA_human,VPM_check,resolve,ready,CS_update process
    class PR_loop,PR_mode,PR_stop,PR_next,VA_check,route,HO_decision,CS_check decision
